                   ...
                   int flip_flag, int mirror_flag)
{
    ...
    if(fread(YUV_buffer,nWidth * nHeight * 3/2,sizeof(unsigned char),fp) != sizeof(unsigned char))
    {
        printf("fread error!");
    }
    fclose(fp);
...
BOOL ReadYUV420File(const char inputfilename[256], int nWidth, int nHeight, char *yuv_image)
{
    ...
    if(fread(yuv_image, nWidth * nHeight, sizeof(unsigned char), fp) != sizeof(unsigned char))
    {
        printf("fread failed!");
    }
    if(fread(uv_plane,  nWidth * nHeight / 2, sizeof(unsigned char), fp) != sizeof(unsigned char))
    {
        printf("fread failed!");
    }
    fclose(fp);
